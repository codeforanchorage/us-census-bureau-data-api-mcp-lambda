// Ported from upstream PR #141 (uscensusbureau/us-census-bureau-data-api-mcp,
// migration 1773071716018). Re-timestamped so it sorts after the cache
// migrations (1784007265666/7) already applied on prod -- node-pg-migrate
// refuses out-of-order migrations, the same adaptation PR #12 made.
//
// Adds the list_survey_programs() / list_survey_components(TEXT) search
// functions, the components.frequency / frequency_notes columns, and
// backfills frequency + program descriptions from the (updated)
// components-programs.csv for rows already seeded. The backfill is a
// no-op on an empty database; the regular seed pipeline covers fresh
// installs.
